...
// All players (socket.id -> player)
const players = {};
let hostId = null;          // Host socket.id
let deck = [];
let communityCards = [];
let stage = 'idle';
let dealerSeat = null;
    ...
    io.sockets.sockets.forEach((socket, id) => {
        ...
        const you = self
            ? {
                ...
                ready: self.ready,
                folded: self.folded,
                holeCards: self.holeCards,     // You always see your own hole cards
                isHost: id === hostId,
        ...
        const others = playerEntries
            ...
            .map(([pid, p]) => ({
                ...
                folded: p.folded,
                isHost: pid === hostId,
                // Do not send opponents' hole cards before showdown
...
function startGameIfReady() {
    ...
    broadcastMessage(`Players ${activesAll.length} all ready, deal hole cards!`);
    broadcastState();
}
...
function nextStage() {
    if (stage === 'preflop') {
        ...
        stage = 'flop';
        broadcastMessage('Entering flop stage.');
    } else if (stage === 'flop') {
        burnCard();
        communityCards.push(deck.pop());
        stage = 'turn';
        broadcastMessage('Entering turn stage.');
    } else if (stage === 'turn') {
        burnCard();
        communityCards.push(deck.pop());
        stage = 'river';
        broadcastMessage('Entering river stage.');
    } else if (stage === 'river') {
        stage = 'showdown';
        broadcastMessage('Entering showdown stage.');
        distributePayouts();
    } else {
        return;
    }
...
function resetGame() {
    ...
    currentTurnId = null;
    pots = [];
    broadcastMessage('Game reset. Everyone can ready up again.');
    broadcastState();
}
...
io.on('connection', (socket) => {
    ...
    socket.on('join', (data) => {
        ...
        if (players[socket.id]) {
            ...
            if (!hostId) {
                hostId = socket.id;
                broadcastMessage(`"${name}" joined (currently host; can control hand flow).`);
            } else {
    ...
    socket.on('setReady', (ready) => {
        ...
        if (!p) return;
        p.ready = !!ready;
        broadcastMessage(`"${p.name}" ${p.ready ? 'is ready' : 'is unready'}.`);
        broadcastState();
        startGameIfReady();
    });

    socket.on('startNextHand', () => {
        if (socket.id !== hostId) { sendError(socket, 'Only host can start next hand'); return; }
        ...
        currentTurnId = null;
        pots = [];
        broadcastMessage('Next hand opened. All players: click "Ready" to start dealing.');
        broadcastState();
    });
    ...
    socket.on('fold', () => {
        const p = players[socket.id];
        if (!p) return;
        if (stage === 'idle' || stage === 'showdown') { sendError(socket, 'Fold not allowed in current stage'); return; }
        if (socket.id !== currentTurnId) { sendError(socket, 'Not your turn yet'); return; }
        p.folded = true;
        p.inHand = false;
        p.actedThisRound = true;
        advanceTurn();
        broadcastMessage(`"${p.name}" folds.`);
        broadcastState();
    });
    ...
    socket.on('bet', (amount) => {
        const p = players[socket.id];
        if (!p) return;
        if (stage === 'idle' || stage === 'showdown') { sendError(socket, 'Bet not allowed in current stage'); return; }
        if (socket.id !== currentTurnId) { sendError(socket, 'Not your turn yet'); return; }
        const a = Math.floor(Number(amount) || 0);
        if (currentMaxBet > 0) { sendError(socket, 'Bet already placed this round; use Raise'); return; }
        if (a < minBet) { sendError(socket, `Minimum bet is ${minBet}`); return; }
        p.currentBet += a;
        p.totalCommitted += a;
        ...
        updatePots();
        advanceTurn();
        broadcastMessage(`"${p.name}" bets ${a}.`);
        broadcastState();
    });
    ...
    socket.on('call', () => {
        const p = players[socket.id];
        if (!p) return;
        if (stage === 'idle' || stage === 'showdown') { sendError(socket, 'Call not allowed in current stage'); return; }
        if (socket.id !== currentTurnId) { sendError(socket, 'Not your turn yet'); return; }
        const need = Math.max(0, currentMaxBet - (p.currentBet || 0));
        if (need <= 0) { sendError(socket, 'Nothing to call'); return; }
        p.currentBet += need;
        p.totalCommitted += need;
        p.actedThisRound = true;
        updatePots();
        advanceTurn();
        broadcastMessage(`"${p.name}" calls ${need}.`);
        broadcastState();
    });
    ...
    socket.on('check', () => {
        const p = players[socket.id];
        if (!p) return;
        if (stage === 'idle' || stage === 'showdown') { sendError(socket, 'Check not allowed in current stage'); return; }
        if (socket.id !== currentTurnId) { sendError(socket, 'Not your turn yet'); return; }
        if ((p.currentBet || 0) !== currentMaxBet) { sendError(socket, 'Higher bet present; cannot check'); return; }
        p.actedThisRound = true;
        advanceTurn();
        broadcastMessage(`"${p.name}" checks.`);
        broadcastState();
    });
    ...
    socket.on('raise', (amount) => {
        const p = players[socket.id];
        if (!p) return;
        if (stage === 'idle' || stage === 'showdown') { sendError(socket, 'Raise not allowed in current stage'); return; }
        if (socket.id !== currentTurnId) { sendError(socket, 'Not your turn yet'); return; }
        const a = Math.floor(Number(amount) || 0);
        const needToCall = Math.max(0, currentMaxBet - (p.currentBet || 0));
        const inc = a - needToCall;
        if (needToCall < 0) { sendError(socket, 'Invalid amount'); return; }
        const finalBet = (p.currentBet || 0) + a;
        if (currentMaxBet > 0 && finalBet < currentMaxBet + minRaiseInc) { sendError(socket, `Min raise to ${currentMaxBet + minRaiseInc}`); return; }
        if (currentMaxBet === 0 && a < minBet) { sendError(socket, `Minimum bet is ${minBet}`); return; }
        p.currentBet += a;
        p.totalCommitted += a;
        currentMaxBet = p.currentBet;
        ...
        updatePots();
        advanceTurn(true);
        broadcastMessage(`"${p.name}" raises to ${p.currentBet}.`);
        broadcastState();
    });
    ...
    socket.on('disconnect', () => {
        ...
        if (p) {
            broadcastMessage(`"${p.name}" disconnected.`);
        }
        if (hostId === socket.id) {
            ...
            if (hostId) {
                ...
                broadcastMessage(`Host changed to "${newHost.name}".`);
            } else {
                deck = [];
                communityCards = [];
                stage = 'idle';
